 ...
 *
 * More details can be found here: http://wiki.osdev.org/GDT
 */
...
void gdt_init()
{
    // Selector 0x00 is unusable
    // initGDT[0] = struct GDT {.base=0, .limit=0, .type=0};
    // Selector 0x08 is code
    // initGDT[1] = {.base=0x04000000, .limit=0x03ffffff, .type=0x9A};
    // Selector 0x10 is data
    // initGDT[2] = {.base=0x08000000, .limit=0x03ffffff, .type=0x92};

	memset(&gdt, 0, sizeof(struct GDT) * 3);
	next_entry = 1;

	new_entry(0, 0xFFFFF, GDT_PRESENT | GDT_DPL0 | GDT_CODE | GDT_READABLE,
	          GDT_GRANULAR | GDT_32BIT);

	new_entry(0, 0xFFFFF, GDT_PRESENT | GDT_DPL0 | GDT_DATA | GDT_WRITABLE,
	          GDT_GRANULAR | GDT_32BIT);
